...
- (void)setupFlowItemViews
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        .widthIs(self.view.bounds.size.width/2-6)
        .heightIs(((self.view.bounds.size.height-(self.view.bounds.size.height/4))-128)/2);
        [temp addObject:view];
    }
    
    // 关键步骤：设置类似collectionView的展示效果
    [_flowItemContentView setupAutoWidthFlowItems:[temp copy] withPerRowItemsCount:2 verticalMargin:2 horizontalMargin:2 verticalEdgeInset:2 horizontalEdgeInset:2];
}
